...
- **사이드바 트리** — 등록한 게이트웨이가 각각 최상단 노드로 표시(주소 노출, 우클릭 메뉴에서 자격증명 관리)
  → 태그 → 엔드포인트, `Models` 노드로 DTO 목록. 메서드별 색상 아이콘.
...
스펙 URL 접근 자체에 로그인이 필요한 경우(브라우저가 아이디/비밀번호를 묻고 `401` 반환 — HTTP Basic 인증)엔
트리의 **게이트웨이 노드를 우클릭 → 아이디/비밀번호 설정**(또는 팔레트 명령)을 실행하세요.
SecretStorage에 저장되어 `Authorization: Basic …` 으로 전송됩니다. 자격증명은 **호스트별**로 저장되며,
트리에 🔑 표시와 저장된 아이디가 나타납니다. 같은 로그인을 쓰는 게이트웨이엔 팔레트 명령으로 한 번에 적용할 수 있습니다.
